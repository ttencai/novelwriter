...
export const worldModelZhMessages = {
  ...
  'worldModel.entityChange.title': '发现更新',
  'worldModel.entityChange.pending': '待确认',
  'worldModel.entityChange.pendingCount': ({ count }) => `${String(count ?? 0)} 条角色更新待确认`,
  'worldModel.entityChange.chapter': ({ chapter }) => `来自第 ${String(chapter ?? 0)} 章`,
  'worldModel.entityChange.replaceValue': ({ old, value }) => `${String(old ?? '')} → ${String(value ?? '')}`,
  'worldModel.entityChange.appendValue': ({ value }) => `追加：${String(value ?? '')}`,
  'worldModel.entityChange.descriptionAppend': ({ value }) => `补充描述：${String(value ?? '')}`,
  'worldModel.entityChange.apply': '采纳',
  'worldModel.entityChange.reject': '拒绝',
  'worldModel.entityChange.actionFailed': '处理失败，请重试',
...
export const continuationZhMessages = {
  ...
  'continuation.setup.mode': '处理方式',
  'continuation.setup.mode.continue': '续写',
  'continuation.setup.mode.continueDescription': '按上下文推进剧情',
  'continuation.setup.mode.polish': '偏润色',
  'continuation.setup.mode.polishDescription': '按要求润色草稿',
  'continuation.setup.polishInstruction': '草稿与要求',
  'continuation.setup.polishInstructionPlaceholder': '粘贴草稿，可在前后补充润色、扩写或精简要求',
  'continuation.setup.polish': '润色',
  ...
  'studio.rail.atlasUpdatesPending': '有角色新增或变动待确认',
...
export const novelEnMessages = {
  ...
  'worldModel.entityChange.title': 'Update found',
  'worldModel.entityChange.pending': 'Pending',
  'worldModel.entityChange.pendingCount': ({ count }) => `${String(count ?? 0)} character updates pending`,
  'worldModel.entityChange.chapter': ({ chapter }) => `From chapter ${String(chapter ?? 0)}`,
  'worldModel.entityChange.replaceValue': ({ old, value }) => `${String(old ?? '')} → ${String(value ?? '')}`,
  'worldModel.entityChange.appendValue': ({ value }) => `Append: ${String(value ?? '')}`,
  'worldModel.entityChange.descriptionAppend': ({ value }) => `Description: ${String(value ?? '')}`,
  'worldModel.entityChange.apply': 'Apply',
  'worldModel.entityChange.reject': 'Reject',
  'worldModel.entityChange.actionFailed': 'Could not process this update. Try again.',
  ...
  'continuation.setup.mode': 'Mode',
  'continuation.setup.mode.continue': 'Continue',
  'continuation.setup.mode.continueDescription': 'Advance from context',
  'continuation.setup.mode.polish': 'Polish',
  'continuation.setup.mode.polishDescription': 'Revise the draft',
  'continuation.setup.polishInstruction': 'Draft and instructions',
  'continuation.setup.polishInstructionPlaceholder': 'Paste the draft and add any polishing, expansion, or shortening instructions',
  'continuation.setup.polish': 'Polish',
  ...
  'studio.rail.atlasUpdatesPending': 'Character additions or changes are waiting for review',
