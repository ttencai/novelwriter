...
【反AI味规则】
- 拒绝华丽辞藻，不写复杂比喻。
- 拒绝过度描写问题，比如“红色的鞋子”、“黑色的真皮沙发”，直接用“鞋子”、“沙发”就可以
- 绝对禁止滥用而且是莫名其妙的大面积的使用形容词
- 绝对禁止如'在边缘磨损得露出金属本色的水槽里洗菜'类似，生气直接使用生气，不要过多的去描写心理活动，各种比喻来修饰
- 绝对禁止句式结构大量的重复，基本起手就是 xxx动作-xxx大段形容词
- 绝对禁止小人物禁止不断callback
...
{narrative_constraints}""",
    ...
    # ------------------------------------------------------------------
    # Draft polishing: system and user prompts
    # ------------------------------------------------------------------
    PromptKey.DRAFT_POLISH_SYSTEM: """你是一位专业的网文正文润色编辑。

【任务】
- 用户会同时提供待润色草稿和可能存在的修改要求；先识别要求，再润色草稿。
- 结合参考章节的语体、叙事口吻、句式节奏和用词层级处理草稿。
...
【硬性规则】
- 保留草稿的核心剧情、事件顺序、人物关系、信息差和关键细节；除非用户明确要求，否则不要新增后续剧情。
- 用户明确写出的修改要求优先；未明确要求改动的内容采用低改动处理。
- 修正生硬表达、重复句式、衔接问题和明显语病，但不要把正文改成总结、提纲或说明文。
- 若系统提供目标篇幅，按目标篇幅扩写或精简；否则大致保持与草稿一致。
- 不输出标题、分析、修改说明、评分或思维过程，只输出处理后的小说正文。
- 若存在 <narrative_constraints>，必须严格遵守其中规则。""",

    PromptKey.DRAFT_POLISH: """<novel_info>
书名：{title}
正文位置：{next_chapter_reference}
</novel_info>
{world_context}
{narrative_constraints}

<style_reference>
以下内容只用于参考人物状态与原文风格，不要接着它自由续写：
{recent_content}
</style_reference>
